...
async def set_next_pos(position,index,swarm:Swarm):
    """
    Befor set the new position change the position from meteres to deg
    """
...
async def flocking(index,future_position:DronePosition,swarm:Swarm):
        ...
        if other !=index:
           ...
           # NEIGHBOR RADIUS is the maximum distance that can exist between the drones to affect the algorithm
            ...
            if num_neighbors>0:
                ...
                cohesion=(average_position-start_position)*Constants.COHESION_FACTOR
                #calculing alignement values
                alignement=average_velocity*Constants.ALIGNEMENT_FACTOR
                separation*=Constants.SEPARATION_FACTOR
...
async def start_flocking(next_position:DronePosition,swarm:Swarm):
    """
    Testing the algorithm with an example destination
    the algorithm is reloaded eery time the swarm moves 10m attending 7 seconds
    """
    data_collector=[]
    counter_move=0
    ...
    while (temp_position.latitude_deg< next_position.latitude_deg and temp_position.longitude_deg< next_position.longitude_deg):
        ...
        for i in range(Constants.NUM_DRONES):
            await flocking(i,temp_position,swarm)
        ...
        counter_move+=1
        """
        after 8 steps the test stops and 
